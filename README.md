...
如果您想要编译为独立可执行文件：
```bash
pip install nuitka  # 用于将 Python 代码编译为原生可执行文件
```
...
编译完成后，可执行文件位于 `dist_local\main.dist\P2P Screen Sharing.exe`

#### 手动 Nuitka 编译
```bash
nuitka --standalone --windows-disable-console --enable-plugin=tk-inter --enable-plugin=numpy main.py
```

### GitHub Actions 自动构建
项目配置了 GitHub Actions，每次推送代码会自动：
1. 使用 Nuitka 编译为 Windows 可执行文件
2. 使用 Inno Setup 创建安装程序
3. 生成 `P2P-Screen-Sharing-Setup.exe` 安装包
